...
var Lenis = class {
  ...
  scrollTo(target, { offset = 0, immediate = false, lock = false, duration = this.options.duration, easing = this.options.easing, lerp: lerp2 = this.options.lerp, onStart, onComplete, force = false, programmatic = true, userData = {} } = {}) {
    ...
    if (typeof target === "string" || target instanceof HTMLElement) {
      ...
      if (this.options.wrapper !== window) {
        ...
        target = elementRect.top - wrapperRect.top + this.actualScroll;
      } else {
        const rect = element.getBoundingClientRect();
        target = rect.top + this.actualScroll;
      }
  ...
  checkNestedScroll(element, { deltaX, deltaY }) {
    if (Math.abs(deltaX) > Math.abs(deltaY)) {
      if (Math.abs(deltaX) < 1e-3) return false;
      const scrollLeft = element.scrollLeft;
      const scrollWidth = element.scrollWidth;
      const clientWidth = element.clientWidth;
      const isScrollableX = scrollWidth > clientWidth;
      if (!isScrollableX) return false;
      if (deltaX < 0 && scrollLeft === 0) return false;
      if (deltaX > 0 && Math.ceil(scrollLeft + clientWidth) >= scrollWidth) return false;
      return true;
    } else {
      if (Math.abs(deltaY) < 1e-3) return false;
      ...
      if (deltaY > 0 && Math.ceil(scrollTop + clientHeight) >= scrollHeight) return false;
      return true;
    }
  }
  /**
   ...
   */
  get limit() {
    return this.options.orientation === "horizontal" ? this.dimensions.limit.x : this.dimensions.limit.y;
  }
  /**
